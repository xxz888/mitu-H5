	...
	<view class="content">
		<view class="body">
			<view class="header-top">
				<p @click="$emit('update:pickerTime',false )">取消</p>
				<view class="selc">
					<b>制定还款日期</b>
					<a>
						（至少选择{{days}}天）
				...
				<p @click="$emit('update:pickerTime',false )">确定</p>
			</view>
			<view class="head-body">
				<text class="date-head">{{swiperData.year}}年{{swiperData.month}}月</text>
				<view class="rig">
					<p @click='prevDeta'>上个月</p>
					<p @click='nextDate'>下个月</p>
					<!-- <view class='pr40' style="display: inline-block;"><uni-icon type="arrowup" size='26' color='#fff' /></view> -->
					<!-- <view style="display: inline-block;"><uni-icon type="arrowdown" size='26' color='#fff'  /></view> -->
				</view>
			</view>
	...
	export default {
		data() {
			return {
				...
				day: '',
				storageDate: [],
				days:uni.getStorageSync("last_day")
				
			}
		},

		onShow() {
			console.log("days===========",uni.getStorageSync("last_day"))
		},
		props: {
			// 当前日期
			date: {
