<template>
	<view class="container">
		<view class="header">
			...
			<h1>我的卡包</h1>
			<h2>
				<p @click="change(item.type)" v-for="(item,index) in typeList" :key="index"
					:class="item.type == type?'active':''">{{item.title}}</p>
			</h2>
		</view>
		<view class="empty" v-if="creditcardList.length == 0&&type == 1">
			<view><image :src="empty" mode="widthFix"></image></view>
			<view class="tits">{{message}}</view>
			
		</view>
		<view class="empty" v-if="debitcardList.length == 0&&type == 2">
			<view><image :src="empty" mode="widthFix"></image></view>
			<view class="tits">{{message}}</view>
			
		</view> 
		<scroll-view v-if="type == 1" style="height: 60vh ;margin-top: 20upx" scroll-y="true">
		<ul >
			<li v-for="(item,index) in creditcardList" :key="index" >
				...
				<view class="card-top-part">
					...
					<h1>{{item.bankName}}</h1>
					<h2>{{item.fullname.substring(0,1)+'*'+item.fullname.substring(2,3)}} <text>尾号：{{item.cardNo.substring(item.cardNo.length,item.cardNo.length-4)}}</text></h2>
				
					<image src="/static/images/yl-icon@2x.png" mode="widthFix" class="logo"></image>
					<b class="edit-btn"  @click="gomodifies(item.id)">
						<image src="/static/images/xg.png" mode="widthFix"></image>
						修改
					</b>
					<b class="del-btn" @click="deleteXyCard(item.id)">
						<image src="/static/images/sc.png" mode="widthFix"></image>
						删除
					</b>
					<!-- <a v-if="item.def">默认卡</a>
					<a v-else @click="changexyCard(item.id)">设置默认卡</a> -->
					
				</view>
				<view class="card-btm-part">
					<p>账单日期：{{item.billDay}}日</p>
					<p>还款日期：{{item.repaymentDay}}日</p>
					<p>距离还款日期：{{item.howRepayment}}日</p>
				</view>
			</li>
			
		</ul>
		</scroll-view>
		<scroll-view v-else style="height: 60vh ;margin-top: 20upx" scroll-y="true">
		<ul >
			<li v-for="(item,index) in debitcardList" :key="index" style="border-radius: 20upx;">
				<view class="card-top-part" >
					<image :src="'../../static/images/banklogo/BANK_'+item.bankAcronym+'.png'" mode="widthFix" class="logo-icon"></image>
					<h1>{{item.bankName}}</h1>
					<h2>{{item.fullname.substring(0,1)+'*'+item.fullname.substring(2,3)}} <text>尾号：{{item.cardNo.substring(item.cardNo.length,item.cardNo.length-4)}}</text></h2>
					<h3 v-if="item.def">默认卡</h3>
					<h3 v-else @click="setDefaultCard(item.id)">设置默认卡</h3>
					<b class="edit-btn" @click="godebitCard(item.id)">
						<image src="/static/images/xg.png" mode="widthFix"></image>
						修改
					</b>
					<b class="del-btn" @click="deletedebitCard(item.id)">
						<image src="/static/images/sc.png" mode="widthFix"></image>
						删除
					</b>
	...
	export default {
		...
		methods: {
			...
			changexyCard(e){
				...
				this.$api.api_changexyCard(body).then(res=>{
			...
			deletedebitCard(id){
				...
				uni.showModal({
					...
					success(res) {
						if(res.confirm){
							t.$api.api_deletecxCard(body).then(res=>{
			...
			setDefaultCard(id){
				...
				this.$api.api_setDefaultCard(body).then(res=>{
			...
			getxinyongcard(){
				...
				let t = this
				let body = {}
				this.$api.api_creditCard(body).then(res=>{
			...
			getdebitcard(){
				let t = this
				let body = {}
				this.$api.api_debitCard(body).then(res=>{	
