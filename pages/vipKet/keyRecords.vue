...
<!-- 		<view class="tab-box">
			<h1 :class="type=='1'?'active':''" @click="choose(1)">未使用 (18) </h1>
			<h1 :class="type=='2'?'active':''"  @click="choose(2)">已使用(15)</h1>
		</view> -->
		<view class="main-box">
<!--  -->
...
<!-- 		key	权益码
			id	信息id
			createTime 创建时间 -->
			<h6 class="ysy" v-for="(item,index) in keyusedList" :key="index">
				<view>
					钻石会员权益 <p>权益码：{{item.key}}</p>
				</view>
				<view>
					平台发放 <p  style="color:#999999 ;">编号：{{item.id}}</p><a>{{item.createTime}}</a>
				</view>
			</h6>
	...
	export default {
		...
		methods: {
			...
			userKeyGet(index){
				...
				this.$meapi.api_userKeGget(body).then(res=>{
