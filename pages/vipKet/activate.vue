...
<!-- 		<view class="empty" v-if="myteams.length == 0">
		...
		<view class="tab-box">
			<h1 :class="type=='1'?'active':''" @click="choose(1)">未使用 ({{usedCount1}}) </h1>
			<h1 :class="type=='2'?'active':''"  @click="choose(2)">已使用({{usedCount2}})</h1>
		</view>
		<view class="main-box">
		<block v-if="type==2">
			<ul>
				<li v-for="(item,index) in keyusedList2" :key="index">
					<view class="user-box">
						<image src="/static/head.jpg"></image>
						<view class="right-box">
							<h1>
								<p v-if="item.fullname==''||item.fullname==undefined">未实名</p>
								<p v-else>{{item.fullname.substring(0,1)+'*'+item.fullname.substring(item.fullname.length,item.fullname.length-1)}} 
								
								<text v-if="item.vipLevel==0">普通</text>
								<text v-else>钻石{{item.vipLevel}}</text>
								
								</p>
								<p @click="call(item.username)">
									{{item.username.substring(0,3)+'****'+item.username.substring(8,11)}}
									<image src="/static/images/kef/phone.png" mode="widthFix"></image>
								</p>
							</h1>
							<h1>
								<!-- <a></a> -->
								<a>激活时间:{{item.createTime}}</a>
							</h1>
						</view>
					</view>
					<view class="data-box">
		...
		<block v-if="type==1" >
			<h6 class="ysy" v-for="(item,index) in  keyusedList1" :key="index">
				<p>
					<view class="v1" >
						<view style="font-size: 20upx;">钻石会员权益</view>
						<view class="vv2" >平台购买</view>
					</view>
					<view class="v2">
						<view style="font-size: 20upx;">激活码：{{item.key}}</view>
						<view class="v22">有效期：{{item.invalidTime.substring(0,10)}}</view>
					</view>
				</p>
				<a @click="fuzi(item.key)">
					复制
				</a>
			</h6>
	...
	export default {
		...
		methods: {
			...
			keyUsed(index){
				...
				this.$meapi.api_userKey(body).then(res=>{
			...
			userKeyunused(index){
				...
				this.$meapi.api_userKeyunused(body).then(res=>{
...
<style lang="scss">
	...
	.tab-box {
		...
		.active {
			border-bottom: 5upx solid rgba(216, 160, 103, 1);
		}
