<template>
	<view class="container">
		...
		<view class="empty" v-if="myteams.length == 0">
			<view><image :src="empty" mode="widthFix"></image></view>
			<view class="tits">{{message}}</view>
			
		</view>
		<view class="main-box">
			<view class="search-content" >
				<view class="search-box">
					<image src="/static/images/kef/sec@2x.png" mode="widthFix"></image>
					<input type="text" v-model="fullname" placeholder="请输入手机号或者姓名" />
					<p @click="getDirectList(1)">搜索</p>
				</view>
				<!-- <picker :range="years" @change="yearChange" mode="multiSelector">
					<view class="select-box">
						<p>{{selectValue}}</p>
						<image src="/static/images/kef/arrb@2x.png" mode="widthFix"></image>
					</view>
				</picker> -->
	<!-- 			<view class="select-box" @click="getDirectList(0)">
					全部
				</view> -->
			</view>
			<ul>
<!-- 					fullname		用户姓名
					...
					createTime	注册时间
					firstCount		直推个数
				firstActivationCount		直推激活个数
				teamCount			团队人数
				curMonthAll		当月总收入 -->
				
						
				
				<li v-for="(item,index) in myteams" :key="index" @click="getlist(item.id)">
					<picker mode = "selector"  @change="bindDateChange" @cancel="cancel" :range="array">
					<view class="user-box">
						<image src="/static/head.jpg"></image>
						<view class="right-box">
							<h1>
								<p>
								<block v-if="item.fullname==undefined">未实名</block> 
								<block v-else>{{item.fullname.substring(0,1)+'*'+item.fullname.substring(item.fullname.length,item.fullname.length-1)}}</block>
																
								<!-- <text>{{item.fullname}}</text> -->
								<!-- <text v-if="item.vipLevel!=0">v{{item.vipLevel}}</text>
								<text v-else>普通</text> -->
								
								<text v-if="item.vipLevel > 0 && item.vipLevel <= 3">钻石{{item.vipLevel}}</text>
								<text v-else-if="item.vipLevel > 3">钻石3</text>
								<text v-else>普通</text>
								<text @click="">调级</text>
								</p>
								<p @click="call(item.username)">
									{{item.username.substring(0,3)}}****{{item.username.substring(7,11)}}
									<image src="/static/images/kef/phone.png" mode="widthFix"></image>
								</p>
							</h1>
							<h1>
								<a></a>
								<a>注册时间:{{item.createTime}}</a>
							</h1>
						</view>
					</view>
					<view class="data-box">
						<p>
							直推人数：
							<text>{{item.firstCount}}</text>
						</p>
						<p>
							团队人数:
							<text>{{item.teamCount}}</text>
						</p>
						<p>
							直推激活：
							<text>{{item.firstActivationCount}}</text>
						</p>
						<p>
							本月交易量(元)：
							<text>{{item.curMonthAllAmount}}</text>
						</p>
					</view>
					</picker>
				</li>
		...
		<!-- <view class="picker">
			...
			<view class="body">
				<scroll-view scroll-y="true" style="height: 30rpx;" bindscrolltoupper="upper" bindscrolltolower="lower" bindscroll="scroll">
				  <view id="demo1" class="scroll-item" v-for="(item,index) in array" :key="index">{{item}}</view>
				</scroll-view>
				<swiper>
					<block >
	...
	export default {
		...
		methods: {
			...
			bindDateChange(e){
				...
				let vipName = this.array[e.detail.value];
				let vipLevel = vipName == '钻石1'? 1 :  vipName == '钻石2'? 2 : vipName == '钻石3'? 3 : "";
				...
				let body = {subUserId:this.selectId,vipLevel:vipLevel};
				this.$api.api_confirmLevel(body).then(res => {
				   this.getDirectList(0,this.selectValueIndex)
			...
			getlist(index){
				this.selectId = index;
			   this.array = [];
				this.$api.api_tiaojiList({id:index}).then(res => {
					if(res.code == 0){
						for(var i=0;i<res.data.length;i++){
							this.array.push(res.data[i].levelName)
						}
						// this.array = res.data
			...
			getDirectList(index,choose){
				...
				this.$meapi.api_getDirectList(body).then(res=>{
...
<style lang="scss">
	...
	.main-box {
		...
		ul {
			...
			li {
				background: #FFFFFF;
				box-shadow: 0px 4upx 6upx 0px rgba(28, 28, 28, 0.3);
				border-radius: 10upx;
				margin-bottom: 20upx;
				padding: 30upx 10upx;
