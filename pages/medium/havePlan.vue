...
<!-- 				<pick-regions @getRegion="handleGetRegion">
				...
				<view style="border-top: 1upx solid #E5E5E5;margin-bottom: 20upx;"></view>
				<view class="feiluclass">
					<view class="v1">执行费率</view>
					<view class="v2">80+</view>
					<input  type="number" v-model="addRate" maxlength="2" />
					
				</view>
				<h2>
					注：输入20则收取用户的费率为80+20，即每一万元收取100元的手续费
				</h2>
			</view>
	...
	export default {
		...
		methods: {
			...
			planNewMake(){
				...
				this.$api.api_planNewMake(body).then(res=>{
					...
					if(res.code == 0){
						...
						uni.navigateTo({
							url:"showNewPlan?type=goNewPlan&cardId="+t.cardId
						})
					}else{
						uni.showToast({
							title:res.message,
			...
			changeDayNumber(e){
				...
				this.dayRepaymentCount = e.target.value*1+1
				console.log("选择每日还款次数",e.target.value*1+1)
				this.budGetDay()
			},
			// 获取选择的地区
