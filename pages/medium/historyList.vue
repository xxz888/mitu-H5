<template>
	<view class="container" >
		...
		<ul>
			<li v-for="(item,index) in historyList" :key="index" @click="goDetail(item.status ,item.id,index)">
				<h1>
					<image :src="'/static/images/banklogo/BANK_'+ylhk_card.bankAcronym+'.png'" mode="widthFix"></image>
					<b>{{ylhk_card.bankName}}（{{ylhk_card.cardNo.substring(ylhk_card.cardNo.length,ylhk_card.cardNo.length-4)}}）</b>
					<a v-if="item.status==1" style="color: #FF9426;">新建</a>
					<a v-if="item.status==2" style="color: #ff9426;">执行中</a>
					<a v-if="item.status==3" style="color: #209e2f;">已完成</a>
					<a v-if="item.status==4" style="color: red;">还款失败</a>
					<a v-if="item.status==5" style="color: red;">取消中</a>
					<a v-if="item.status==6" style="color: #2741ad;">已取消</a>
					<a v-if="item.status==7" style="color: red;">取消失败</a>
					<a v-if="item.status==8" style="color: #209e2f;">执行完成</a>
				</h1>
				<h2>
					<p>还款笔数</p>
					<p>{{item.completedCount}}笔</p>
				</h2>
				<h2>
					<p>还款类型</p>
					<p>智能还款</p>
				...
				<h2 style="color: #FF9426 ;">
					<p>剩余应还</p>
					<p>{{(item.taskAmount-item.repaymentedAmount).toFixed(2)}}</p>
				</h2>
				<h2>
					<p>创建时间</p>
					<p>{{item.createTime}}</p>
	...
	export default {
		...
		methods: {
			...
			getHistoryList(cardNo){
				...
				this.$api.api_getHistoryList(body).then(res=>{
