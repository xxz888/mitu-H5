		...
		<ul>
			<li v-for="(item,index) in debitcardList" :key="index" style="border-radius: 20upx;">
				...
				<view class="card-top-part"  >
					<image  @click="setDefaultCard(item.id)" :src="'../../../static/images/banklogo/BANK_'+item.bankAcronym+'.png'" mode="widthFix" class="logo-icon"></image>
					<h1  @click="setDefaultCard(item.id)">{{item.bankName}}</h1>
					<h2  @click="setDefaultCard(item.id)">{{item.fullname.substring(0,1)+'*'+item.fullname.substring(2,3)}} <text>尾号：{{item.cardNo.substring(item.cardNo.length,item.cardNo.length-4)}}</text></h2>
				...
				<view class="card-btm-part"  @click="setDefaultCard(item.id)">
					<p>借记卡</p>
