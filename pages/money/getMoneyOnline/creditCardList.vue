<template>
	<view class="container">
		...
		<scroll-view style="height: 70vh ;margin-top: 40upx;" scroll-y="true">
		<ul>
			<li v-for="(item,index) in creditcardList" :key="index">
				...
				<view class="card-top-part">
					<image  @click="changexyCard(item.id)" :src="'../../../static/images/banklogo/BANK_'+item.bankAcronym+'.png'" mode="widthFix" class="logo-icon"></image>
					<h1  @click="changexyCard(item.id)"  >{{item.bankName}}</h1>
					<h2  @click="changexyCard(item.id)">{{item.fullname.substring(0,1)+'*'+item.fullname.substring(2,3)}} <text>尾号：{{item.cardNo.substring(item.cardNo.length,item.cardNo.length-4)}}</text></h2>
				
					<image  @click="changexyCard(item.id)" src="/static/images/yl-icon@2x.png" mode="widthFix" class="logo"></image>
					...
					<b class="del-btn" @click="deleteXyCard(item.id)">
						<image src="/static/images/del-btn.png" mode="widthFix"></image>
						删除
					</b>
				...
				<view class="card-btm-part"  @click="changexyCard(item.id)">
	...
	export default {
		...
		methods: {
			...
			deletedebitCard(id){
				...
				this.$api.api_deletecxCard(body).then(res=>{
